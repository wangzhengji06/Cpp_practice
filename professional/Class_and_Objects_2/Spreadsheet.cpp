...
void Spreadsheet::Impl::verifyCoordinate(size_t x, size_t y) const {
  ...
  if (y >= m_height) {
    throw out_of_range{
        format("x ({}) must be less than height ({}).", y, m_height)};
  }
}

void Spreadsheet::Impl::setCellAt(size_t x, size_t y,
...
size_t Spreadsheet::Impl::getId() const { return m_id; }

// Spreadsheet member function definition
...
size_t Spreadsheet::getId() const { return m_impl->getId(); }
